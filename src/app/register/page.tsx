...
import { getDisplayErrorMessage, resendVerificationEmail } from "@/lib/api";
...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setPending(true);
    try {
      const result = await signUpWithPassword(u, em, password);
  ...
  async function handleResend() {
    ...
    try {
      const res = await resendVerificationEmail(pendingEmail);
      setResendMessage(res.message);
    } catch (err) {
      setResendMessage(getDisplayErrorMessage(err, t("register.resendFailed")));
    } finally {
    ...
    return (
      <div className="login-page__backdrop relative flex flex-1 items-center justify-center px-6 py-16">
        <div className="w-full max-w-md rounded-2xl border border-white/30 bg-white/90 p-8 shadow-xl backdrop-blur dark:border-zinc-700 dark:bg-zinc-900/90">
          ...
          <p className="mt-3 text-sm text-zinc-500 dark:text-zinc-400">{t("register.checkEmailHint")}</p>
          <button
            type="button"
            onClick={() => void handleResend()}
            disabled={resendPending}
            className="mt-6 w-full rounded-lg border border-zinc-300 px-4 py-2.5 text-sm font-medium text-zinc-800 hover:bg-zinc-50 disabled:opacity-60 dark:border-zinc-600 dark:text-zinc-100 dark:hover:bg-zinc-800"
          >
            {resendPending ? t("register.resending") : t("register.resend")}
          </button>
          {resendMessage ? (
            <p className="mt-3 text-center text-sm text-zinc-600 dark:text-zinc-400" role="status">
              {resendMessage}
            </p>
          ) : null}
