...
import { EMPTY_PUBLIC_STATS, type PublicStatsResponse } from "@/lib/public-stats";

/** Marketing display value for the home stats strip (not live DB count). */
const DISPLAY_CATCHES_LOGGED = 312;
...
/** Server-side platform stats for the marketing home page (cached via page revalidate). */
export async function loadPublicStats(): Promise<PublicStatsResponse> {
  try {
    ...
    return {
      catchesLogged: DISPLAY_CATCHES_LOGGED,
      lakesMapped: Math.max(geo.lakesMapped, pub?.lakesMapped ?? 0, admin?.lakesMapped ?? 0),
      speciesTracked: Math.max(geo.speciesTracked, pub?.speciesTracked ?? 0),
      tripsPlanned: backend?.tripsPlanned ?? 0,
    };
  } catch (e) {
    console.error("[loadPublicStats]", e);
